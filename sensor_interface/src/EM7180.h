...
#define AK8963_XOUTnnnnnn_L 0x03 // data
...
//
#define EM7180_QX                                                              \
  0x00 // this is a 32-bit normalized floating point number read from registers
       // 0x00-03
#define EM7180_QY                                                              \
  0x04 // this is a 32-bit normalized floating point number read from registers
       // 0x04-07
#define EM7180_QZ                                                              \
  0x08 // this is a 32-bit normalized floating point number read from registers
       // 0x08-0B
#define EM7180_QW                                                              \
  0x0C // this is a 32-bit normalized floating point number read from registers
       // 0x0C-0F
#define EM7180_QTIME                                                           \
  0x10 // this is a 16-bit unsigned integer read from registers 0x10-11
...
#define EM7180_Baro                                                            \
  0x2A // start of two-byte MS5637 pressure data, 16-bit signed interger
#define EM7180_BaroTIME                                                        \
  0x2C // start of two-byte MS5637 pressure timestamp, 16-bit unsigned
#define EM7180_Temp                                                            \
  0x2E // start of two-byte MS5637 temperature data, 16-bit signed interger
#define EM7180_TempTIME                                                        \
  0x30 // start of two-byte MS5637 temperature timestamp, 16-bit unsigned
...
#define M24512DFM_DATA_ADDRESS                                                 \
  0x50 // Address of the 500 page M24512DRC EEPROM data buffer, 1024 bits (128
       // 8-bit bytes) per page
#define M24512DFM_IDPAGE_ADDRESS                                               \
  0x58 // Address of the single M24512DRC lockable EEPROM ID page
...
uint8_t SBy = t_62_5ms; // set pressure amd temperature output data rate
...
int16_t dig_T2, dig_T3, dig_P2, dig_P3, dig_P4, dig_P5, dig_P6, dig_P7, dig_P8,
    dig_P9;
double Temperature,
    Pressure; // stores BMP280 pressures sensor pressure and temperature
int32_t rawPress,
    rawTemp; // pressure and temperature raw count output for BMP280
...
float gyroBias[3] = {0, 0, 0}, accelBias[3] = {0, 0, 0}, magBias[3] = {0, 0, 0},
      magScale[3] = {0, 0, 0}; // Bias corrections for gyro, accelerometer, mag
int16_t tempCount, rawPressure,
    rawTemperature; // pressure, temperature raw count output
float temperature, pressure,
    altitude; // Stores the MPU9250 internal chip temperature in degrees Celsius
...
uint16_t EM7180_mag_fs, EM7180_acc_fs,
...
void EM7180_set_mag_acc_FS(uint16_t mag_fs, uint16_t acc_fs); // 1000 uT, 8 g
void EM7180_set_gyro_FS(uint16_t gyro_fs);                    // 2000 dps

uint8_t M24512DFMreadByte(uint8_t device_address, uint8_t data_address1,
                          uint8_t data_address2);

void MPU9250SelfTest(
    float *destination); // Start by performing self test and reporting values
...
void accelgyrocalMPU9250(float *dest1, float *dest2); // Calibrate gyro and
